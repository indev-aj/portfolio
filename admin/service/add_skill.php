...
if (isset($_REQUEST)) {
    ...
    $type = !empty($_POST["skill-type"]) ? $conn->real_escape_string($_POST['skill-type']) : "";
    echo "Skill: " . $type;

    // Get uploaded image
    if (isset($_FILES["thumbnail"]) && $_FILES["thumbnail"]["error"] == UPLOAD_ERR_OK) {
        // thumbnail's folder
        $target_dir = "../thumbnail/" . $user['id'] . "/skill/$title/";
    ...
    // Add data into db
    $sql = "INSERT INTO Skill (user_id, name, type, icon) VALUES (?,?,?,?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("dsss", $user['id'], $title, $type, $target_file);

    if ($stmt->execute()) {
        $stmt->close();

        header("Location: https://indevtechnology.com/portfolio/admin/skill.php");
        exit();
    } else {
